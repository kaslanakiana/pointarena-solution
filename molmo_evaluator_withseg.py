...
def call_mask2former(image_path, device=None):
    try:
        image = Image.open(image_path).convert("RGB")

        model_dir = "mask2former-swin-large-coco-panoptic"
        ...
        colored = np.zeros((panoptic_map.shape[0], panoptic_map.shape[1], 3), dtype=np.uint8)
        ...
        mask_image = Image.fromarray(colored)

        return mask_image, True

    except Exception as e:
        print(f"Error in call_mask2former: {e}")
...
MOLMO_MODELS = ["Molmo-7B-D-0924", "Molmo-7B-O-0924", "Molmo-72B-0924","Molmo2-8B"]
...
def initialize_molmo(model_name="allenai/Molmo2-8B"):
    ...
    if molmo_model is None or molmo_processor is None:
        ...
        # Processor is CPU-side and does not need device_map/torch_dtype.
        ...
        if gpu_count > 1:
            # Multi-GPU: let accelerate shard the model automatically.
            ...
            # Single-GPU: first try full placement on cuda:0 to avoid meta/offload mismatch.
            try:
                molmo_model = AutoModelForImageTextToText.from_pretrained(
                    model_source,
                    trust_remote_code=True,
                ...
                # Fallback: 4-bit quantization for smaller single cards.
                print("Single-GPU full precision load OOM. Falling back to 4-bit quantization.")
...
def _get_model_input_device(model):
    """Pick the correct entry device for model inputs across single/multi-GPU layouts."""
...
def _is_multi_gpu_sharded(model):
    """Return True when model weights are split across 2+ CUDA devices."""
    hf_map = getattr(model, "hf_device_map", None)
    if not hf_map:
        return False
    cuda_devices = set()
    for dev in hf_map.values():
        if isinstance(dev, int):
            cuda_devices.add(dev)
        elif isinstance(dev, str) and dev.startswith("cuda:"):
            try:
                cuda_devices.add(int(dev.split(":")[1]))
            except Exception:
                pass
        elif isinstance(dev, torch.device) and dev.type == "cuda":
            cuda_devices.add(dev.index if dev.index is not None else 0)
    return len(cuda_devices) > 1


def _prepare_inputs_for_inference(inputs, model):
    """
    Move inputs safely for single/multi-GPU inference.
    For sharded multi-GPU models, keep index tensors on CPU to avoid
    embedding index device mismatches (e.g. embedding on cuda:7).
    """
    if _is_multi_gpu_sharded(model):
        prepared = {}
        for k, v in inputs.items():
            if torch.is_tensor(v) and v.dtype in (torch.int8, torch.int16, torch.int32, torch.int64, torch.uint8, torch.bool):
                prepared[k] = v.cpu()
    ...
    return {k: (v.to(input_device) if torch.is_tensor(v) else v) for k, v in inputs.items()}
...
POINTS_REGEX = re.compile(r"([0-9]+)\s+([0-9]+(?:\.[0-9]+)?)\s+([0-9]+(?:\.[0-9]+)?)")
...
def extract_points(text, image_w, image_h, extract_ids=False, only_first_frame=True):
    ...
    for coord in COORD_REGEX.finditer(text):
        for point_grp in FRAME_REGEX.finditer(coord.group(1)):
            ...
            if only_first_frame and frame_id != 1:
                continue
...
def add_reference_point_to_image(image, original_points):
    """
    Draw steerable reference points on the image before inference.
    """
    image = image.copy()
    ...
    for point in original_points:
        ...
        # convert percentage → pixel if needed
        ...
        # draw a strong red dot
        r = max(6, min(img_width, img_height) // 80)
...
def call_molmo(image_path, object_name, model_name="allenai/Molmo2-8B", category=None):
    ...
    try:
        ...
        mask_image, _ = call_mask2former(image_path, device)
        if category == "steerable":
            image = add_reference_point_to_image(image, IMAGE_POINTS_MAP[os.path.basename(image_path)])
        img_width, img_height = image.size
        ...
        if category == "counting":
            prompt = f"""这两张图片中第一张为原图，第二张是全景分割后生成的图片。第二张图为提示作用，在原图中完成point任务。
            pointing: {object_name}
            {original_points_info}
            """
        else:
            prompt = f"""这两张图片中第一张为原图，第二张是全景分割后生成的图片。第二张图为提示作用，在原图中完成point任务。只需给出符合条件一个点即可。
            pointing: {object_name}
            {original_points_info}
            """
        messages = [
        ...
        inputs = _prepare_inputs_for_inference(inputs, model)
        ...
        if extracted_points:
            ...
            points = [{"point": [float(x), float(y)]} for frame_id, x, y in extracted_points]
...
def evaluate_model(model_name, model_type, progress_callback=None, resume=True):
    ...
    for i, item in enumerate(data):
        ...
        try:
            ...
            points, model_output = model_func(image_path, query, model_name, category)
